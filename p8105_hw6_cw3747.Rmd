...
Across 100 Monte Carlo splits, the model with head circumference, length, sex, and their interactions had the lowest mean cross-validated prediction error, followed by the simple model with length and gestational age. My proposed model based on maternal characteristics had the largest MSPE, indicating worse predictive performance, although it may still be useful for understanding associations with maternal factors.

The interaction model probably does best because it uses variables that directly describe the baby’s size at birth (head circumference and length), and it allows the relationship between these measures and birthweight to differ by sex. The interaction terms make the model more flexible, so it can fit different “patterns” of birthweight for boys and girls instead of forcing a single straight line for everyone. Since we evaluate performance by cross-validated MSPE rather than in-sample error, this better fit seems to carry over to new data rather than just overfitting the training set.
